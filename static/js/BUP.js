...
$.fn.BUP = function (brother, beishu) {
  ...
  // 
	...
	$blowupLens.css({
		...
		'border-radius': $options.round ? '150%' : 'none',
  ...
  // 鼠标移动时
    ...
    // 缩放后的X坐标
		var zoomX = -Math.floor(
			(relX / $element.width()) * NATIVE_IMG.width / .3 - $options.width / 2
    )
    // 缩放后的Y坐标
		var zoomY = -Math.floor(
			(relY / $element.height()) * NATIVE_IMG.height / .2 - $options.height / 2
		)
		console.log(relX+"/"+$element.width()+"*"+ NATIVE_IMG.width / .5 );
		console.log(zoomX);
		console.log(zoomY);
		var bgSize
		switch (beishu) {
